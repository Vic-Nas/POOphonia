 ...
 * items, such as Song, Album, and Podcast, based on a CSV string.
 *
...
public class MusicItemFactory {
     ...
     * - parts[0]: The type of the music item (song, album, podcast) - parts[1]:
     * The ID (integer) - parts[2]: The title (string) - parts[3]: The release
     * year (integer) - Additional fields depend on the item type: * Song:
     * artist, genre, duration * Album: artist, label, numberOfTracks * Podcast:
     * host, topic, episodeNumber
     * @return a MusicItem instance (Song, Album, or Podcast) if valid,
     * otherwise null.
     */
    public static MusicItem createFromCSV(String[] parts) {
        /**
         * Each music item type (song, album, podcast) requires exactly **7
         * fields** in the CSV format. If fewer than 7 fields are provided, the
         * data is incomplete, and we return null.
         */
        if (parts.length != 7) {
            return null;
        }
